...
class TGFileUtil {
  ...
  static Future<Map<String, dynamic>> readYamlFile(String yamlFile, {String? group, String? subGroup}) async {
    try {
      ...
      for (MapEntry<String, dynamic> entry in map![group].entries) groupMap[entry.key] = entry.value;

      if (subGroup == null || subGroup.length == 0) return groupMap;

      // Sub Group entries
      Map<String, dynamic> subGroupMap = <String, dynamic>{};
      for (MapEntry<String, dynamic> entry in groupMap[subGroup].entries) subGroupMap[entry.key] = entry.value;
